...
static void LoadAssets(DXSample* const sample)
{
	...
	hr = D3DCompileFromFile(shadersPath, NULL, NULL, "VSMain", "vs_5_0", compileFlags, 0, &vertexShader, NULL);
	if (FAILED(hr)) LogErrAndExit(hr);
	...
	hr = ID3D12Device_CreateCommandList(sample->device, 0, D3D12_COMMAND_LIST_TYPE_DIRECT, sample->commandAllocator, sample->pipelineState, IID(&sample->commandList), &sample->commandList);
	if (FAILED(hr)) LogErrAndExit(hr);
